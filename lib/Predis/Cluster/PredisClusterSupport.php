...
/**
 * Support class for the standard client-side cluster provided by Predis.
 *
 * @author Daniele Alessandri <suppakilla@gmail.com>
 */
class PredisClusterSupport implements ClusterSupportInterface
{
    private $commands;
    ...
    protected function getKeyFromZsetAggregationCommands(CommandInterface $command)
    {
        ...
        if ($this->checkSameHashForKeys($key)) {
            return $arguments[0];
        }
    }
    ...
    {
        if ($extractor = $this->commands[$command->getId()]) {
            if ($key = call_user_func($extractor, $command)) {
